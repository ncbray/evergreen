...
func TestForwardEdgeLoop(t *testing.T) {
	...
	if !info[n2].isHead {
		t.Errorf("Not loop head - %d", n2)
	}

	if !info[n2].isIrreducible {
		t.Errorf("Not irreducible - %d", n2)
	}
}

func TestBackEdgeSkip(t *testing.T) {
	g := CreateGraph()
	e := g.Entry()
	x := g.Exit()
	n1 := g.CreateNode()
	...
	// TODO more robust checking.
	if !info[n1].isHead {
		t.Errorf("Not loop head - %d", n1)
	}
	if !info[n2].isHead {
		t.Errorf("Not loop head - %d", n2)
	}
	if !info[n3].isHead {
		t.Errorf("Not loop head - %d", n3)
	}
	if !info[n4].isHead {
		t.Errorf("Not loop head - %d", n4)
	}
}
