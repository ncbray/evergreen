...
func SingleLineComment(frame *runtime.State) {
	var c0 rune
	var c1 rune
	var checkpoint int
	var c2 rune
...
func sInsert(frame *runtime.State) {
	var checkpoint int
	var c rune
	goto block1
block1:
		...
		if frame.Flow == 0 {
			if c9 == '{' {
				...
				if frame.Flow == 0 {
					if c10 == '}' {
						frame.Consume()
						e1 = &Construct{Type: t1, Args: args0}
						goto block5
					}
					frame.Fail()
					goto block2
		...
		if frame.Flow == 0 {
			if c11 == '{' {
				...
				if frame.Flow == 0 {
					if c12 == '}' {
						frame.Consume()
						e1 = &ConstructList{Type: t2, Args: args1}
						goto block5
					}
					frame.Fail()
					goto block3
...
block3:
	frame.Recover(checkpoint)
	e2 = StringMatchExpr(frame)
	if frame.Flow == 0 {
		e1 = e2
		goto block5
	}
	frame.Recover(checkpoint)
	e3 = RuneMatchExpr(frame)
	if frame.Flow == 0 {
		e1 = e3
		goto block5
	}
	...
	if frame.Flow == 0 {
		if c0 == '(' {
			...
			if frame.Flow == 0 {
				if c1 == ')' {
					frame.Consume()
					e2 = &Call{Expr: e1, Pos: pos, Args: args}
					goto block4
				}
				frame.Fail()
				goto block2
...
block2:
	...
	if frame.Flow == 0 {
		if c2 == '.' {
			...
			name = Ident(frame)
			if frame.Flow == 0 {
				e2 = &Selector{Expr: e1, Pos: pos, Name: name}
				goto block4
			}
			goto block3
		}
	...
	if frame.Flow == 0 {
		if c3 == '<' {
			...
			if frame.Flow == 0 {
				if c4 == '>' {
					frame.Consume()
					e2 = &SpecializeTemplate{Expr: e1, Pos: pos, Types: types}
					goto block4
				}
				frame.Fail()
				e3 = e1
...
block3:
	frame.LookaheadFail(checkpoint2)
	frame.Recover(checkpoint1)
	checkpoint3 = frame.LookaheadBegin()
	frame.Peek()
	if frame.Flow == 0 {
		frame.Consume()
		frame.LookaheadFail(checkpoint3)
		frame.Recover(checkpoint0)
		checkpoint4 = frame.Checkpoint()
		SingleLineComment(frame)
		if frame.Flow == 0 {
			goto block4
		}
		frame.Recover(checkpoint4)
		goto block4
	}
	frame.LookaheadNormal(checkpoint3)
	goto block5
block4:
	LineTerminator(frame)
	if frame.Flow == 0 {
