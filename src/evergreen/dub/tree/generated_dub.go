...
block3:
	...
	if frame.Flow == 0 {
		if c3 == '\'' {
			frame.Consume()
			ret0, ret1 = value, frame.Slice(pos)
			return
		}
		frame.Fail()
		goto block4
																							...
																							if frame.Flow == 0 {
																								...
																								if frame.Flow == 0 {
																									...
																									if frame.Flow == 0 {
																										if c30 == ')' {
																											frame.Consume()
																											ret = &Assign{Expr: &Append{List: &NameRef{Name: r4}, Expr: r5}, Targets: []ASTExpr{&NameRef{Name: r4}}}
																											goto block9
																										}
																										frame.Fail()
																										goto block4
						...
						if frame.Flow == 0 {
							...
							if frame.Flow == 0 {
								...
								if frame.Flow == 0 {
									if c36 == '}' {
										frame.Consume()
										ret = &ConstructList{Type: r10, Args: r11}
										goto block9
									}
									frame.Fail()
									goto block7
...
block7:
	frame.Recover(checkpoint)
	r12 = StringMatchExpr(frame)
	if frame.Flow == 0 {
		ret = r12
		goto block9
	}
	frame.Recover(checkpoint)
	r13 = RuneMatchExpr(frame)
	if frame.Flow == 0 {
		ret = r13
		goto block9
	}
